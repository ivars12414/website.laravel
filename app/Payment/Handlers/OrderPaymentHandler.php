...
//                insertTelegramNotification($tgStartText . implode(". ", $tgText));
                ...
                $orderService->updateOrderStatus($order, $statuses['completed'], $source);

                break;
            case 'refunded':
                $orderService->updateOrderStatus($order, $statuses['canceled'], $source, ['reason_hash' => StatusReason::findByLabel('refund', $statuses['canceled'])->hash]);
                break;
            case 'canceled':
                $orderService->updateOrderStatus($order, $statuses['canceled'], $source, ['reason_hash' => StatusReason::findByLabel('by_client', $statuses['canceled'])->hash]);
                break;
            case 'declined':
                $orderService->updateOrderStatus($order, $statuses['canceled'], $source, ['reason_hash' => StatusReason::findByLabel('declined', $statuses['canceled'])->hash]);
                break;
        }
    }
}
